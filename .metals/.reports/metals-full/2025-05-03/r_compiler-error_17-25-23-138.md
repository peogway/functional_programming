...
// Monitor function (display data)
  ...
  def search(data: List[String]): Map[Date, Double] = {
    ...
    val date = readLine()
    Try(inputFormat.parse(date)) match {
      case Failure(_) =>
        println("Invalid date format. Please use yyyy/MM/dd.")
        return Map.empty[Date, Double]
      case Success(parsedDate) =>
        val searchDate = inputFormat.format(parsedDate)
        ...
        val newData =
          data.drop(1).foldLeft(Map.empty[Date, Double]) { (acc, line) =>
            ...
            println(searchDate)
            println(outputFormat.parse(timestamp))
            println(@@)
            if (outputFormat.parse(timestamp) == searchDate) {
